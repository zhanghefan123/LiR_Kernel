...
struct RoutingTableEntry *init_routing_table_entry(int bloom_filter_total_length) {
    struct RoutingTableEntry *routing_table_entry = (struct RoutingTableEntry *) (kmalloc(
            sizeof(struct RoutingTableEntry), GFP_KERNEL));
    routing_table_entry->bitset = (unsigned long *) (kmalloc(sizeof(unsigned long) * bloom_filter_total_length,
                                                             GFP_KERNEL));
    ...
    while (true) {
        ...
        if (single_number == NULL || (strcmp(single_number, "") == 0)) {
            ...
            if (count == 0) {
                ...
                if (count == 3) {
                    struct NewInterfaceEntry output_interface_entry = find_entry_in_new_interface_table(
                            new_interface_table, number);
...
struct RoutingTableEntry *get_routing_table_entry_by_iph(struct net *current_net_namespace, struct iphdr *iph) {
    ...
    struct hlist_head *lir_routing_table = get_lir_routing_table_from_net_namespace(
            current_net_namespace); // 从网络命名空间之中获取路由表
    ...
    routing_table_entry = find_entry_in_routing_table(lir_routing_table, current_satellite_id,
                                                      source_satellite_id); // 根据源卫星查找路由
    return routing_table_entry;
}
...
get_routing_table_entry_by_sock_opt(struct net *current_net_namespace, struct ip_options_rcu *opt) {
    struct RoutingTableEntry *routing_table_entry; // 结果路由表项
...
struct LirReturnDataStructure get_destination_and_find_routing_table_entry(struct net* current_net_namespace, struct ip_options_rcu* opt){
    ...
    // not set bloom filter
    lir_return_data_structure.routing_table_entry = lir_routing_entry;
    lir_return_data_structure.bloom_filter = net_bloom_filter; // the bitset is not set
    lir_return_data_structure.output_interface = output_interface;
    return lir_return_data_structure;
}

//struct LirReturnDataStructure get_destination_and_construct_path_and_proofs_bf(struct net *current_net_namespace,
//                                                                               struct ip_options_rcu *opt,
//                                                                               unsigned char* static_field_hash) {
//    struct LirReturnDataStructure lir_return_data_structure = {
//            .output_interface = NULL
//    };
//    struct net_device *output_interface;
...
//            u32 final_insert_element;
//            int intermediate_node = lir_routing_entry->node_ids[index]; // find intermediate satellite
//            char key_from_source_to_intermediate[20];  // key from source to intermediate satellite
...
//            }
//            push_element_into_bloom_filter(net_bloom_filter, &final_insert_element, sizeof(u32));
//        }
//        count++;
//    }
//    // 拷贝一份布隆过滤器并进行返回
...
get_destination_list_and_construct_bf(struct net *current_net_namespace, struct ip_options_rcu *opt) {
    struct LirReturnDataStructure lir_return_data_structure = {
            .output_interface = NULL
    };
    struct net_device *output_interface;
 ...
 */
void
rebuild_routing_table_with_new_bf_settings(struct bloom_filter *bloom_filter, struct hlist_head *lir_routing_table) {
    struct hlist_head *hash_bucket = NULL;
    struct RoutingTableEntry *current_entry = NULL;
    struct hlist_node *next;
    int index;
    ...
    for (index = 0; index < BUCKET_COUNT; index++) {
        ...
        hlist_for_each_entry_safe(current_entry, next, hash_bucket, pointer) {
            ...
            // 重新根据布隆过滤器进行计算
            push_elements_into_bloom_filter(bloom_filter, current_entry->length_of_path,
                                            current_entry->link_identifiers);
            // 计算出了新的布隆过滤器之后进行拷贝

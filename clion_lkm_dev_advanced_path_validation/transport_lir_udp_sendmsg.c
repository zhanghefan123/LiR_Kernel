...
int lir_udp_sendmsg(struct sock *sk, struct msghdr *msg, size_t len){
    ...
    // --------------       get route        --------------
    lir_return_data_structure = get_destination_and_find_routing_table_entry(current_net_namespace, inet->inet_opt);
    destination_node_id = lir_return_data_structure.destination_node_id;
    // saddr = lir_return_data_structure.output_interface->ip_ptr->ifa_list->ifa_address; // output interface address is ip address
    // --------------       get route        --------------

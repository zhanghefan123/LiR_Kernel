...
int retrieve_new_interface_table_handler(struct sk_buff* request, struct genl_info* info){
    ...
    struct net *current_net_namespace = sock_net(request->sk);
    // 获取接口表
    struct NewInterfaceTable* interface_table = get_new_interface_table_from_net_namespace(current_net_namespace);
    // 响应的报文
    struct sk_buff *reply;
    // 缓存 - 用户空间下发的数据
    char *buffer;
    ...
    sprintf(response_msg, "new interface table:\n");
    for(int index = 0; index < interface_table->number_of_interfaces; index++){
        sprintf(single_entry_in_str,"(%s)\n", interface_table->interface_entry_array[index].interface->name);
        strcat(response_msg, single_entry_in_str);
    }
    if (0 != nla_put_string(reply, EXMPL_NLA_DATA, response_msg)) {
        return -EINVAL;
    }
...
int get_bind_id_handler(struct sk_buff* request, struct genl_info* info){
    ...
    sprintf(response_msg, "current satellite id: %d", get_satellite_id(current_net_namespace));
    if (0 != nla_put_string(reply, EXMPL_NLA_DATA, response_msg)) {
        return -EINVAL;
    }
    if (0 != nla_put_u32(reply, EXMPL_NLA_LEN, 1)) {

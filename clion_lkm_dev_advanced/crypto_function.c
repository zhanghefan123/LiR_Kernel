...
unsigned char* calculate_hmac(struct shash_desc* hmac_data_structure, char* data, char* key){
    int pre_bytes = 4;
    unsigned char hmac_output[20];
    unsigned char* real_output = (unsigned char*)(kmalloc(sizeof(unsigned char) * pre_bytes, GFP_KERNEL));
    // set key in hmac
    ...
    // calculate digest
    if (crypto_shash_digest(hmac_data_structure, data, strlen(data), hmac_output)) {
    ...
    memcpy(real_output, hmac_output, sizeof(unsigned char) * pre_bytes);
    return real_output;
}
